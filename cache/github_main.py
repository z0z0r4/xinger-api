                ...
                if cache_dir_info_resp.status_code == 200:
                    for file_info in cache_dir_info_resp.json()["tree"]:
                        if file_info["type"] == "blob":
                            cache_image_results.append(f'https://testingcf.jsdelivr.net/gh/{owner}/{repo}@raw/main/{path}/{file_info["path"]}')
...
async def image(type: str = None):
    ...
    if len(github_img_url_cache) != 0:
        img_url = random.choice(github_img_url_cache)
        if type is None:
